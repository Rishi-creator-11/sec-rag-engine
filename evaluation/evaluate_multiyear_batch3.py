"""Phase 5.5 Batch 3 multi-year structural evaluation — WMT / UNH / XOM.
...
``benchmark_multiyear_batch3.json``.
...
and coverage checks that are read from the same evidence set.
    ...
    python -m evaluation.evaluate_multiyear_batch3
"""
...
BENCHMARK = REPO_ROOT / "evaluation" / "benchmark_multiyear_batch3.json"
OUT_JSON = REPO_ROOT / "evaluation" / "results" / "multiyear_batch3_evaluation.json"
OUT_TXT = REPO_ROOT / "evaluation" / "results" / "multiyear_batch3_summary.txt"
...
def main() -> int:
    ...
    lines = [
        "MULTI-YEAR (FISCAL-YEAR SCOPE) EVALUATION - Phase 5.5 Batch 3 (WMT/UNH/XOM)",
